...
class ContractServiceImpTest {
  ...
  void saveContract_shouldReturnGetContract() {
    ...
    when(converter.postContractToEntity(postContract))
      .thenReturn(savedContract);
    when(contractRepository.save(savedContract)).thenReturn(savedContract);
    when(converter.entityToGetContract(savedContract))
      .thenReturn(expectedGetContract);
  ...
  void getAllContracts_shouldReturnListOfGetContracts() {
    ...
    List<GetContract> expectedGetContracts = List.of(
      new GetContract(),
      new GetContract()
    );
    ...
    when(converter.entityToGetContract(any(Contract.class)))
      .thenReturn(expectedGetContracts.get(0), expectedGetContracts.get(1));

    List<GetContract> result = contractService.getAllContracts(
      pageSize,
      pageNumber
    );
